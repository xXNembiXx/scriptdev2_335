...
struct MANGOS_DLL_DECL boss_anubrekhanAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            m_creature->MonsterTextEmote("Anub'Rekhan beginnt damit, einen Schwarm Insekten zu entfesseln!", 0, true);
        ...
        {
            if (Creature* pTemp = m_creature->SummonCreature(NPC_CRYPT_GUARD, m_afCryptGuardPos[0], m_afCryptGuardPos[1], m_afCryptGuardPos[2], m_afCryptGuardPos[3], TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 60000))
            {
                pTemp->MonsterTextEmote("Eine Gruftwache greift in den Kampf ein!", 0, true);
                m_lCryptGuardList.push_back(pTemp->GetGUID());
...
struct MANGOS_DLL_DECL mob_crypt_guardAI : public ScriptedAI
{
    ...
    void KilledUnit(Unit* pVictim)
    {
        ...
        if (urand(0, 1))
            ...
            {
                pVictim->CastSpell(pVictim, SPELL_SELF_SPAWN_5, true);
                m_creature->MonsterTextEmote("Aus dem Leichnam von %s's krabbeln Leichenskarabäen!", 0, true);
            }
    }
